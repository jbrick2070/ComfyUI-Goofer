"""
Goofer_AudioEnhance — Upscales mono 24kHz TTS audio to stereo 48kHz with
...
log = logging.getLogger("Goofer.AudioEnhance")
...
class GooferAudioEnhance:
    ...
    def enhance(self, audio, target_sample_rate=48000, spatial_width=0.3,
        ...
        if waveform is None:
            log.warning("Goofer_AudioEnhance: no waveform data, passing through")
            return (audio,)
        ...
        log.info("Goofer_AudioEnhance: input %dHz %dch %d samples (%.1fs)",
        ...
        log.info("Goofer_AudioEnhance: output %dHz %dch %d samples (%.1fs) — "
