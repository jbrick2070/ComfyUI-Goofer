"""
GooferBatchVideo — Generate up to 5 LTX-2 AV video clips in one node execution.

Forked from DMM_BatchVideoGenerator by Jeffrey A. Brick.
...
log = logging.getLogger("Goofer.BatchVideo")
...
def _vram_lock():
    ...
    if not _lock_checked:
        try:
            ...
            _lock_works = True
            _lock_checked = True
            log.info("GooferBatchVideo: push_model_lock() available — VRAM locking enabled")
            with ctx:
                yield
            return
            ...
            _lock_works = False
            _lock_checked = True
            log.info("GooferBatchVideo: push_model_lock() unavailable (%s) — using pre-load fallback", e)
            yield
            return
...
class GooferBatchVideo:
    ...
    def generate(
        ...
        width, height, length, fps, crf, vram_resident="disabled",
        ...
        all_prompts = [prompt_1, prompt_2, prompt_3, prompt_4, prompt_5][:batch_size]
        # Filter out empty prompt slots (PromptGen pads with "" for unused slots)
        prompts = [p for p in all_prompts if p.strip()]
        if not prompts:
            prompts = ["Cinematic aerial view of a modern city, establishing shot"]
        n = len(prompts)

        # Collect i2v conditioning images + strengths only for non-empty prompt slots
        all_images = [image_1, image_2, image_3, image_4, image_5][:batch_size]
        all_strengths = [cond_strength_1, cond_strength_2, cond_strength_3,
                         cond_strength_4, cond_strength_5][:batch_size]
        # Keep only entries whose corresponding prompt was non-empty
        images = [all_images[i] for i, p in enumerate(all_prompts) if p.strip()][:n]
        cond_strengths = [all_strengths[i] for i, p in enumerate(all_prompts) if p.strip()][:n]
    ...
    def _run_pipeline(
        ...
        if torch.cuda.is_available():
            ...
            try:
                ...
                pass  # ipc_collect not available on all builds
        log.info("VRAM cleanup complete")
